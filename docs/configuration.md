...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, per-task timeline records (`data/<id>/timeline.json`, schema owned by `bin/fm_task_timeline.py`), and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
FM_SILENT_IDLE_SECS=7200           # seconds a window may sit idle (not busy) with no terminal status line and no declared paused:/captain-held line before bin/fm-watch.sh's silent_idle_check surfaces one silent-idle stale wake for that idle stretch; independent of the wedge-escalation cascade above so it still fires when that cascade's own per-hash state has gone quiet; inspection-only, never a teardown or steer; 0 disables it
