...
claude, codex, opencode, pi, pi-signed, grok, kimi, and cursor are empirically verified for crewmate and secondmate launches, while muse and agy are verified for crewmate and scout launches only and are refused for a secondmate; [README requirements](../README.md#requirements) own the set supported for the primary session.
