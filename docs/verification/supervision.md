...
Session-lock ownership in `bin/fm-session-lock-lib.sh` is decided against a session's whole contiguous harness ancestry rather than one chosen pid, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare`/`bg-pty-host`/`daemon run` hook worker chain, or an inner pid when a harness-named daemon parents the session.
The walk passes through every `daemon run`/`bg-spare`-shaped layer instead of stopping at the first one it meets, because Claude Code 2.1.280 (measured live 2026-09-22, main home) inserted a second such layer - `bg-spare` - below `bg-pty-host`, so the FIRST harness match a hook shell's ancestry walk reaches can now itself be an excluded layer; stopping there left `fm_session_lock_owned_by_self` false and froze the Stop-owned auto-arm ledger on `outcome=arming` until a brand-new session happened to relaunch it.
...
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table and runs the real Stop auto-arm in version-named, daemon-parented, combined, and stacked bg-spare-below-bg-pty-host real and simulated process trees.
