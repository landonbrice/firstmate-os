...
Independently of that hash-stability cascade, a `silent-idle` backstop watches plain wall-clock idle duration: once a window has been observed not busy for `FM_SILENT_IDLE_SECS` (default 7200) with no terminal status line (`done:`/`needs-decision:`/`blocked:`/`failed:`) and no declared `paused:` or `captain-held` line, `bin/fm-watch.sh`'s `silent_idle_check` surfaces exactly one stale wake for that idle stretch (inspect only, never a teardown or steer), then resets when the pane goes busy again or a terminal or paused line appears.
It exists as a duration-based check independent of pane-hash churn or escalation counters, so it still fires where the hash-stability cascade's own per-hash state has gone quiet, catching a worker that finished only through chat or an interactive pane menu and never wrote a terminal status line (the 2026-08-29 silent-done strand).
