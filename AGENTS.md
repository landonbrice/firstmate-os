...
config/              local operating choices; LOCAL, gitignored; docs/configuration.md owns every key, its accepted values, its default, and whether a secondmate home inherits it
data/                personal fleet records; LOCAL, gitignored as a whole
  backlog.md         task queue, dependencies, history (section 10)
  captain.md         this home's domain-local captain preferences and working style; canonical even if harness memory mirrors it, and updated with inspect-then-update
  captain-shared.md  main-authoritative shared captain preferences propagated read-only to secondmate homes; owned by secondmate-provisioning
  learnings.md       fleet-local operational facts and gotchas; dated, evidence-backed, curated, and updated with inspect-then-update - rewrite and prune rather than append forever, the same contract as captain.md; created lazily, absent until this home has a learning to store
...
state/               runtime records and signals; gitignored; docs/configuration.md and each producing script's header own the per-file inventory
  <id>.status        appended by crewmates: "<state>: <note>" wake-event lines, not current-state truth
  <id>.meta          task metadata; each producer script's header owns its exact fields and mutation contract
  <id>.inbox/        durable steering inbox written by fm-send, with ordinary records re-rung and escalated by the watcher while explicit fire-and-forget records are excluded from that ladder, and removed by teardown (bin/fm-task-inbox-lib.sh)
  <id>.check.sh      authenticated slow poll; the watcher runs registered checks from hash-validated private snapshots and rejects every other state check without execution
...
Never hand-edit the watcher's own internals (`.hash-*`, `.count-*`, `.stale-*`, `.stale-since-*`, `.paused-*`, `.wedge-escalations-*`, `.writing-*`, `.seen-*`, `.hb-surfaced-*`, `.last-*`, `.heartbeat-streak`, `.watch.lock`, `.wake-queue.lock`, `.last-watcher-beat`), the generation-bound recovery state in `.watcher-down`, the per-actor wake-row claims (`.branch-eligible-rows`, `.branch-eligible-owner`, `.main-eligible-rows`), the Claude Stop auto-arm and Cursor stop-hook owner records (`.claude-autoarm-*`, `.turnend-claude-blocks*`, `.cursor-park-owner*`, `.turnend-cursor-blocks`), or the sub-supervisor internals (`.subsuper-*`, `.supervise-daemon.*`).
Every other `state/` and `config/` entry is owned by `docs/configuration.md`, by the doc its own line names, and by the header of the script that writes it; read those rather than inferring a file's contract from its name.
