...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/timeline.json per-task time and token timeline record, written by spawn and teardown checkpoints and read by the bridge console's `t` key; survives teardown; bin/fm_task_timeline.py owns its schema
