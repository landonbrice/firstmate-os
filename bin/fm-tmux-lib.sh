...
# separated composer shape, tmux's analogue of herdr's native `agent get`. It
# answers only for the harnesses that draw that shape - pi and agy - from two
# live signals:
...
#     runtime-backends.md "Agent liveness name sources") or an agy process,
#     falling back to tmux's own foreground-derived #{pane_current_command}.
#     A pane whose agent died to a shell has no such foreground process and
#     gets NO identity, which is exactly what keeps the strict blank-row rule
#     honest: a blank row between two stale rules stays unknown.
...
# agy's process name is matched exactly rather than globbed, matching the
# anchoring rule in bin/fm-harness.sh and bin/backends/tmux.sh.
